/****
 * source code for warcraft part 3
 * 
 * */
...
// first declare all base classes
class Tribe;    // no derived classes
class Warrier;  // will be inherited by other classes
class Weapon;   // different weapons have different properties, will be inherited by other classes
class City;     // each city has flags, no derived classes
...
class Warrier{
    public:
        Warrier(int id_, int element_):id(id_), element(element_){ }  // constructor
...
class Dragon:public Warrier{
    public:
        ...
        Dragon(int id_, int element_, float morale_):Warrier(id_, element_), morale(morale_), weapon(id_%3){}
        string born_msg(){
            string name("dragon ");
...
class Ninja:public Warrier{
    ...
    public:
        static int ph_cost; // this will be changed for each subtypes accordingly
        Ninja(int id_, int element_):Warrier(id_, element_){
            weapon1 = id%3;
            weapon2 = (id+1)%3;
        }
        string born_msg(){
...
class Iceman:public Warrier{
    ...
    public:
        static int ph_cost; // this will be changed for each subtypes accordingly
        Iceman(int id_, int element_):Warrier(id_, element_), weapon(id_%3){}
        string born_msg(){
            string name("iceman ");
            return name + Warrier::born_msg();
        }
...
class Lion:public Warrier{
    ...
    public:
        static int ph_cost; // this will be changed for each subtypes accordingly    
        Lion(int id_, int element_, int loyalty_):Warrier(id_, element_), loyalty(loyalty_){}
        string born_msg(){
            string name("lion ");
            return name + Warrier::born_msg();
        }
...
class Wolf:public Warrier{
    ...
    public:
        static int ph_cost; // this will be changed for each subtypes accordingly
        Wolf(int id_, int element_):Warrier(id_, element_){}
        string born_extramsg(){ 
            string empty("NULL");
            return empty;
        }
...
class Tribe{
    private:
        ...
        int element;  // remained element of this tribe
        string name;
    ...
    public:
        ...
        Tribe(int element_, const char* name_):element(element_), aid(0), name(name_){
...
bool Tribe::produce(int t, int wtype)
{
    ...
    if(wtype==DRAGON_IDX){
        if(element<Dragon::ph_cost)   return false;
        aid++;
        element -= Dragon::ph_cost;
        w = new Dragon(aid, Dragon::ph_cost, 1.0*element/Dragon::ph_cost);
    }
    else if(wtype==NINJA_IDX){
        if(element<Ninja::ph_cost) return false;
        aid++;
        element -= Ninja::ph_cost;
        w = new Ninja(aid, Ninja::ph_cost);
    }
    else if(wtype==ICEMAN_IDX){
        if(element<Iceman::ph_cost) return false;
        aid++;
        element -= Iceman::ph_cost;
        w = new Iceman(aid, Iceman::ph_cost);
    }
    else if(wtype==LION_IDX){
        if(element<Lion::ph_cost) return false;
        aid++;
        element -= Lion::ph_cost;
        w = new Lion(aid, Lion::ph_cost, element);
    }
    else if(wtype==WOLF_IDX){
        if(element<Wolf::ph_cost) return false;
        aid++;
        element -= Wolf::ph_cost;
...
int main()
{
    ...
    for(int test=0;test<tests;test++){
        ...
        // h-00: both headquaters producing warriers, follow previous assignment
        // h-05: lions escape, remove lion from the warriers matrix
        // h-10: all warriers step forward. iterate over all matrixs, attach each warriers to corresponding cities
        // h-20: generate 10 elements continuously in each city, until the elements are taken away by warriers
        // h-35: warriers with arrow start shoting
        // h-38: warriers with bomb assess whether to use bomb or not
        // h-40: battle between warriers. iterate over all cities 
        // h-50: two tribes report their remaining element, a print out function
        // h-55: all warriers report their equipment, a print out function
